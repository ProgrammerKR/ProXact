...
jest.mock('proxact/jsx-runtime', () => require('./jsx-runtime'), {virtual: true});
jest.mock('proxact/jsx-dev-runtime', () => require('./jsx-dev-runtime'), {
  virtual: true,
});

let React = require('proxact');
let ReactDOM = require('proxact-dom');
let ReactTestUtils = {
...
it('should warn when `key` is being accessed on composite element', () => {
  ...
  expect(() => ReactDOM.render(<Parent />, container)).toErrorDev(
    'Child: `key` is not a prop. Trying to access it will result ' +
      ...
      'prop. (https://proxactjs.org/link/special-props)'
  );
});
  ...
  if (process.env.BABEL_ENV === 'development') {
    expect(() => ReactDOM.render(<ClassParent />, container)).toErrorDev([
      'Warning: Component "ClassWithRenderProp" contains the string ref "myRef". ' +
        ...
        'https://proxactjs.org/link/strict-mode-string-ref',
    ]);
  } else {
