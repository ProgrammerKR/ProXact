...
import {Logger} from 'babel-plugin-proxact-compiler/src/Entrypoint';
      ...
      logEvent: (filename, event): void => {
        ...
        if (event.kind === 'CompileError') {
          ...
          if (__unstable_donotuse_reportAllBailouts && event.fnLoc != null) {
             ...
             * Compiler bailout lints only serve to flag that a proxact function
          ...
          if (
            hasFlowSuppression(detail.loc, 'proxact-rule-hook') ||
            hasFlowSuppression(detail.loc, 'proxact-rule-unsafe-ref')
          ) {
