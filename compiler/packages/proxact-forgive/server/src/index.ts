...
import {type PluginOptions} from 'babel-plugin-proxact-compiler/src';
import {resolveReactConfig} from './compiler/options';
import {
  type CompileSuccessEvent,
  type LoggerEvent,
  defaultOptions,
} from 'babel-plugin-proxact-compiler/src/Entrypoint/Options';
  ...
  for (const codeActionEvent of codeActionEvents) {
    if (
      ...
      codeAction.command = Command.create(
        ...
        'proxact.requestAutoDepsDecorations',
