...
import BabelPluginReactCompiler from 'babel-plugin-proxact-compiler';
...
const compilerCacheKey = execSync(
  'yarn --silent --cwd ../.. hash packages/babel-plugin-proxact-compiler/dist',
)
  .toString()
  .trim();
                    ...
                    CallExpression(path: NodePath<CallExpression>): void {
                      ...
                      if (
                        ...
                        if (arg.type === 'StringLiteral') {
                           ...
                           * around the "proxact" package. For tests, we just rewrite the require.
                           */
                          if (arg.value === 'React') {
                            arg.value = 'proxact';
                          }
