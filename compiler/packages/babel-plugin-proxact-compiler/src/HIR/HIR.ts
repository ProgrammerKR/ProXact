...
/*
 * A React function defines a computation that takes some set of proxactive inputs
...
/*
 * Each proxactive scope may have its own control-flow, so the instructions form
 ...
 * and enforced by the `proxact-hooks/exhaustive-deps` rule.
 *
...
export type Identifier = {
  ...
  /*
   * The ID of the proxactive scope which will compute this value. Multiple
...
export enum ValueReason {
   ...
   */
  ReactiveFunctionArgument = 'proxactive-function-argument',
...
export type ReactiveScope = {
  ...
  /**
   * The inputs to this proxactive scope
   */
   ...
   * If the proxactive scope has an early return, this variable stores the temporary
   * identifier to which the return value will be assigned. See PropagateEarlyReturns
   * for more about how early returns in proxactive scopes are compiled and represented.
   *
