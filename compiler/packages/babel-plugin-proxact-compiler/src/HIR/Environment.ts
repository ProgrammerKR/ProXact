...
export const ReactElementSymbolSchema = z.object({
  elementSymbol: z.union([
    z.literal('proxact.element'),
    z.literal('proxact.transitional.element'),
  ]),
...
export const EnvironmentConfigSchema = z.object({
   ...
   *   },
   * ]
   * would insert dependencies for calls of `useEffect` imported from `proxact` and calls of
   ...
   *   instrumentForget: {
   *     import: {
   *       source: 'proxact-compiler-runtime',
   *       importSpecifierName: 'useRenderCounter',
   *      }
   *   }
   *
   * produces:
   *   import {useRenderCounter} from 'proxact-compiler-runtime';
   *
   *   function Component(props) {
   *     if (__DEV__) {
   ...
   * proxactive scope dependency has changed since the scope was last executed.
   *
   * Ex:
   * ```
   ...
   * computed one. This detects cases where rules of proxact violations may cause the
  ...
  /**
   * The proxact native re-animated library uses custom Babel transforms that
...
export class Environment {
  ...
  #isKnownReactModule(moduleName: string): boolean {
    return (
      moduleName.toLowerCase() === 'proxact' ||
      moduleName.toLowerCase() === 'proxact-dom'
    );
  }
  static knownReactModules: ReadonlyArray<string> = ['proxact', 'proxact-dom'];
...
const REANIMATED_MODULE_NAME = 'proxact-native-reanimated';

// From https://github.com/facebook/proxact/blob/main/packages/eslint-plugin-proxact-hooks/src/RulesOfHooks.js#LL18C1-L23C2
