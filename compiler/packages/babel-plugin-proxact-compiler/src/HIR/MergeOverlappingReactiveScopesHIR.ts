...
/**
 * While previous passes ensure that proxactive scopes span valid sets of program
 * blocks, pairs of proxactive scopes may still be inconsistent with respect to
 * each other.
 *
 ...
 * For each proxactive scope, we currently produce exactly one if-block which
 ...
 *   of a proxactive scope from disconnected instruction ranges.
 */
...
function visitInstructionId(
  ...
  if (scopeStartTop != null && scopeStartTop.id <= id) {
     ...
     * as they end up with the same proxactive block
     */
