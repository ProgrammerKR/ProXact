  ...
  switch (value.kind) {
    ...
    case 'LoadGlobal': {
      ...
      if (hookKind === 'useMemo' || hookKind === 'useCallback') {
        ...
        sidemap.proxact.add(lvalId);
      }
      break;
    }
    case 'PropertyLoad': {
      if (sidemap.proxact.has(value.object.identifier.id)) {
      ...
      if (
        ...
        if (manualMemo != null) {
          ...
          if (isValidationEnabled) {
             ...
             * ```
             * While we could handle this by diffing proxactive scope deps
