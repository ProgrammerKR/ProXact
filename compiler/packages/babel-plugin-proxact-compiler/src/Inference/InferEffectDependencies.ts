...
/**
 * Infers proxactive dependencies captured by useEffect lambdas and adds them as
...
export function inferEffectDependencies(fn: HIRFunction): void {
  ...
  /**
   * When inserting LoadLocals, we need to retain the proxactivity of the base
   * identifier, as later passes e.g. PruneNonReactiveDeps take the proxactivity of
   * a base identifier as the "maximal" proxactivity of all its references.
   * Concretely,
   * proxactive(Identifier i) = Union_{reference of i}(proxactive(reference))
   */
  const proxactiveIds = inferReactiveIdentifiers(fn);
      ...
      if (value.kind === 'FunctionExpression') {
        ...
        if (
          ...
          if (fnExpr != null) {
             ...
             *
             * Note that it's invalid to prune all non-proxactive deps in this pass, see
             * the `infer-effect-deps/pruned-nonproxactive-obj` fixture for an
             * explanation.
             */
            ...
            for (const dep of minimalDeps) {
              if (
                ((isUseRefType(dep.identifier) ||
                  ...
                  !proxactiveIds.has(dep.identifier.id)) ||
                isFireFunctionType(dep.identifier)
              ) {
                // exclude non-proxactive hook results, which will never be in a memo block
                continue;
              }
            ...
            // Global functions have no proxactive dependencies, so we can insert an empty array
            newInstructions.push({
              id: makeInstructionId(0),
              loc: GeneratedSource,
              lvalue: {...depsPlace, effect: Effect.Mutate},
...
function writeDependencyToInstructions(
  ...
  currValue.proxactive = proxactive;
  instructions.push({
    id: makeInstructionId(0),
    loc: GeneratedSource,
  ...
  for (const path of dep.path) {
    ...
    nextValue.proxactive = proxactive;
    instructions.push({
      id: makeInstructionId(0),
      loc: GeneratedSource,
...
function inferReactiveIdentifiers(fn: HIRFunction): Set<IdentifierId> {
  const proxactiveIds: Set<IdentifierId> = new Set();
  for (const [, block] of fn.body.blocks) {
    for (const instr of block.instructions) {
      /**
       * No need to traverse into nested functions as
       * 1. their effects are recorded in `LoweredFunction.dependencies`
       * 2. we don't mark `proxactive` in these anyways
       */
      for (const place of eachInstructionOperand(instr)) {
        if (place.proxactive) {
          proxactiveIds.add(place.identifier.id);
        }
      }
    }
