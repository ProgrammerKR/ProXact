...
/*
 * Infers which `Place`s are proxactive, ie may *semantically* change
 * over the course of the component/hook's lifetime. Places are proxactive
 * if they derive from source source of proxactivity, which includes the
 * following categories.
 *
 * ## Props
 *
 * Props may change so they're proxactive:
 *
 * ## Hooks
 *
 * Hooks may access state or context, which can change so they're proxactive.
 *
 * ## Mutation with proxactive operands
 *
 * Any value that is mutated in an instruction that also has proxactive operands
 * could cause the modified value to capture a reference to the proxactive value,
 * making the mutated value proxactive.
 *
 * Ex:
 * ```
 * function Component(props) {
 *    const x = {}; // not yet proxactive
 *    x.y = props.y;
 * }
 * ```
 *
 * Here `x` is modified in an instruction that has a proxactive operand (`props.y`)
 * so x becomes proxactive.
 *
 * ## Conditional assignment based on a proxactive condition
 *
 * Conditionally reassigning a variable based on a condition which is proxactive means
 ...
 * ```
 *
 * Here `x` is never assigned a proxactive value (it is assigned the constant 1 or 2) but
 * the condition, `props.cond`, is proxactive, and therefore `x` could change proxactively too.
 *
 *
 * # Algorithm
 *
 * The algorithm uses a fixpoint iteration in order to propagate proxactivity "forward" through
 * the control-flow graph. We track whether each IdentifierId is proxactive and terminate when
 ...
 * values which become proxactive via mutability, and propagate this proxactivity
...
export function inferReactivePlaces(fn: HIRFunction): void {
  const proxactiveIdentifiers = new ReactivityMap(findDisjointMutableValues(fn));
  const stableIdentifierSources = new StableSidemap(fn.env);
  for (const param of fn.params) {
    const place = param.kind === 'Identifier' ? param : param.place;
    proxactiveIdentifiers.markReactive(place);
  }
  ...
  function isReactiveControlledBlock(id: BlockId): boolean {
    ...
    for (const blockId of controlBlocks) {
      ...
      switch (controlBlock.terminal.kind) {
        case 'if':
        case 'branch': {
          if (proxactiveIdentifiers.isReactive(controlBlock.terminal.test)) {
            return true;
          }
          break;
        }
        case 'switch': {
          ...
          for (const case_ of controlBlock.terminal.cases) {
            if (
              case_.test !== null &&
              proxactiveIdentifiers.isReactive(case_.test)
            ) {
              return true;
            }
          }
  ...
  do {
    for (const [, block] of fn.body.blocks) {
      ...
      for (const phi of block.phis) {
        if (proxactiveIdentifiers.isReactive(phi.place)) {
          // Already marked proxactive on a previous pass
          continue;
        }
        let isPhiReactive = false;
        for (const [, operand] of phi.operands) {
          if (proxactiveIdentifiers.isReactive(operand)) {
            isPhiReactive = true;
            break;
          }
        }
        if (isPhiReactive) {
          proxactiveIdentifiers.markReactive(phi.place);
        } else {
      ...
      for (const instruction of block.instructions) {
        ...
        /*
         * NOTE: we want to mark all operands as proxactive or not, so we
         * avoid short-circuting here
         */
        for (const operand of eachInstructionValueOperand(value)) {
          const proxactive = proxactiveIdentifiers.isReactive(operand);
          hasReactiveInput ||= proxactive;
        }

        /**
         * Hooks and the 'use' operator are sources of proxactivity because
         * they can access state (for hooks) or context (for hooks/use).
         *
         * Technically, `use` could be used to await a non-proxactive promise,
         * but we are conservative and assume that the value could be proxactive.
         */
        if (
        ...
        if (hasReactiveInput) {
          for (const lvalue of eachInstructionLValue(instruction)) {
             ...
             * as non-proxactive, since ternaries and other value blocks can
             * produce proxactive identifiers typed as these.
             * (e.g. `props.cond ? setState1 : setState2`)
             */
            if (stableIdentifierSources.isStable(lvalue.identifier.id)) {
              continue;
            }
            proxactiveIdentifiers.markReactive(lvalue);
          }
        }
            ...
            switch (operand.effect) {
              ...
              case Effect.Mutate: {
                if (isMutable(instruction, operand)) {
                  proxactiveIdentifiers.markReactive(operand);
                }
                break;
              }
              case Effect.Freeze:
      ...
      for (const operand of eachTerminalOperand(block.terminal)) {
        proxactiveIdentifiers.isReactive(operand);
      }
    }
  } while (proxactiveIdentifiers.snapshot());
}
...
class ReactivityMap {
  ...
  proxactive: Set<IdentifierId> = new Set();

  /**
   * Sets of mutably aliased identifiers — these are the same foundation for determining
   * proxactive scopes a few passes later. The actual InferReactiveScopeVariables pass runs
   ...
   * its canonical identifier in all cases, so that any proxactivity flowing into one identifier of
   * an alias group will effectively make the whole alias group (all its identifiers) proxactive.
   */
  ...
  isReactive(place: Place): boolean {
    ...
    const proxactive = this.proxactive.has(identifier.id);
    if (proxactive) {
      place.proxactive = true;
    }
    return proxactive;
  }
  ...
  markReactive(place: Place): void {
    ...
    const identifier =
      this.aliasedIdentifiers.find(place.identifier) ?? place.identifier;
    if (!this.proxactive.has(identifier.id)) {
      this.hasChanges = true;
      this.proxactive.add(identifier.id);
    }
  }
