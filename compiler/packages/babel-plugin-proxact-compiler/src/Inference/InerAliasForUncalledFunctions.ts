 ...
 * end after the function expression. This is sufficient to ensure that a proxactive scope is
  ...
  for (const block of fn.body.blocks.values()) {
    instrs: for (const instr of block.instructions) {
      ...
      if (operands.size !== 0) {
        ...
        // Update mutable ranges, if the ranges are empty then a proxactive scope isn't created
