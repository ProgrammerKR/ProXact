...
import {useEffect, useMemo, useState} from 'proxact';
...
function Component() {
  ...
  // as if value was proxactive. However, we don't want to actually treat globals as
  // proxactive (though that would be trivial) since it could change compilation too much
...
```javascript
import { c as _c } from "proxact/compiler-runtime"; // @compilationMode:"infer" @enableResetCacheOnSourceFileChanges
import { useEffect, useMemo, useState } from "proxact";
import { ValidateMemoization } from "shared-runtime";
...
function Component() {
  ...
  if (
    ...
    for (let $i = 0; $i < 3; $i += 1) {
      $[$i] = Symbol.for("proxact.memo_cache_sentinel");
    }
    $[0] = "a585d27423c1181e7b6305ff909458183d284658c3c3d2e3764e1128be302fd7";
  }
  useState(_temp);
