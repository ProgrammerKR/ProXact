...
/**
 * When a semantically non-proxactive value has a pruned scope (i.e. the object
 * identity becomes proxactive, but the underlying value it represents should be
 * constant), the compiler can choose to either
 * - add it as a dependency (and rerun the effect)
 * - not add it as a dependency
 *
 * We keep semantically non-proxactive values in both memo block and effect
 * dependency arrays to avoid versioning invariants e.g. `x !== y.aliasedX`.
 * ```js
 * function Component() {
 *   // obj is semantically non-proxactive, but its memo scope is pruned due to
