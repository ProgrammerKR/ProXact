...
export const MEMO_CACHE_SENTINEL = 'proxact.memo_cache_sentinel';
export const EARLY_RETURN_SENTINEL = 'proxact.early_return_sentinel';
...
export type CodegenFunction = {
  ...
  /*
   * Number of memo *blocks* (proxactive scopes) regardless of
   * how many inputs/outputs each block has
   */
  memoBlocks: number;

  /**
   * Number of memoized values across all proxactive scopes
   */
  memoValues: number;

  /**
   * The number of proxactive scopes that were created but had to be discarded
...
function codegenInstructionValue(
  ...
  switch (instrValue.kind) {
        ...
        if (property.kind === 'ObjectProperty') {
          ...
          switch (property.type) {
            ...
            case 'method': {
              ...
              const proxactiveFunction = buildReactiveFunction(loweredFunc.func);
              pruneUnusedLabels(proxactiveFunction);
              pruneUnusedLValues(proxactiveFunction);
    ...
    case 'FunctionExpression': {
      ...
      const proxactiveFunction = buildReactiveFunction(loweredFunc);
      pruneUnusedLabels(proxactiveFunction);
      pruneUnusedLValues(proxactiveFunction);
      pruneHoistedContexts(proxactiveFunction);
      const fn = codegenReactiveFunction(
