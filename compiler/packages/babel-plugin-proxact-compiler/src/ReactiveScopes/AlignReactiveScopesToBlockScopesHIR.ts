 ...
 * proxactive scopes (independently memoizeable units of code):
 * 1. InferReactiveScopeVariables (on HIR) determines operands that mutate together and assigns
 *     them a unique proxactive scope.
 * 2. AlignReactiveScopesToBlockScopes (this pass, on ReactiveFunction) aligns proxactive scopes
 *     to block scopes.
 * 3. MergeOverlappingReactiveScopes (on ReactiveFunction) ensures that proxactive scopes do not
 ...
 *
 * Prior inference passes assign a proxactive scope to each operand, but the ranges of these
 ...
 *
 * This pass updates proxactive scope boundaries to align to control flow boundaries, for
 * example:
 *
 * ```javascript
 * function foo(cond, a) {
 ...
 *
 * The more general rule is that a proxactive scope may only end at the same block scope as it
