...
function isStableRefType(
  ...
  proxactiveIdentifiers: Set<IdentifierId>,
): boolean {
  return isUseRefType(identifier) && !proxactiveIdentifiers.has(identifier.id);
}
/*
 * Computes a set of identifiers which are proxactive, using the analysis previously performed
