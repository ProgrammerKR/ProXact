 ...
 * are actually proxactive or not (ie, whether their value can change over time).
 *
 * This pass prunes dependencies that are guaranteed to be non-proxactive.
 */
export function pruneNonReactiveDependencies(fn: ReactiveFunction): void {
  const proxactiveIdentifiers = collectReactiveIdentifiers(fn);
  visitReactiveFunction(fn, new Visitor(), proxactiveIdentifiers);
}
...
class Visitor extends ReactiveFunctionVisitor<ReactiveIdentifiers> {
  ...
  override visitScope(
    ...
    if (scopeBlock.scope.dependencies.size !== 0) {
      /**
       * If any of a scope's dependencies are proxactive, then all of its
       * outputs will re-evaluate whenever those dependencies change.
       * Mark all of the outputs as proxactive to reflect the fact that
       * they may change in practice based on a proxactive input.
       */
