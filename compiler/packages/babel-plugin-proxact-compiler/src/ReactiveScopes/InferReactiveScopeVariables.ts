 ...
 *     together and assigns them a unique proxactive scope.
 * 2. AlignReactiveScopesToBlockScopes (on ReactiveFunction) aligns proxactive scopes
 *     to block scopes.
 * 3. MergeOverlappingReactiveScopes (on ReactiveFunction) ensures that proxactive
 ...
 *
 * For each mutable variable, infers a proxactive scope which will construct that
 * variable. Variables that co-mutate are assigned to the same proxactive scope.
...
export function inferReactiveScopeVariables(fn: HIRFunction): void {
  /*
   * Represents the set of proxactive scopes as disjoint sets of identifiers
