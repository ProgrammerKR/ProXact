...
/**
 * This pass ensures that proxactive blocks honor the control flow behavior of the
 * original code including early return semantics. Specifically, if a proxactive
 ...
 *
 * The compilation strategy is as follows. For each top-level proxactive scope
 ...
 *   - A `break` to the proxactive scope's label.
 *
 * Finally, CodegenReactiveScope adds an if check following the proxactive scope:
 ...
 * let t0;
 * if (props.cond !== $[0]) {
 *   t0 = Symbol.for('proxact.memo_cache_sentinel');
 *   bb0: {
 *     let x = [];
 *     if (props.cond) {
 *       x.push(12);
 ...
 * if (t0 !== Symbol.for('proxact.memo_cache_sentinel')) {
 *   return t0;
 * }
 * ```
 */
...
type State = {
  /**
   * Are we within a proxactive scope? We use this for two things:
   * - When we find an early return, transform it to an assign+break
   *   only if we're in a proxactive scope
   * - Annotate proxactive scopes that contain early returns...but only
   *   the outermost proxactive scope, we can't do this for nested
   *   scopes.
   */
