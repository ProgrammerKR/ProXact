...
/*
 * This pass prunes proxactive scopes that are not necessary to bound downstream computation.
  ...
  visitValueForMemoization(
    ...
    if (value.kind === 'LoadLocal' && lvalue !== null) {
      ...
      if (getHookKind(state.env, callee.identifier) != null) {
         ...
         * to proxactive scopes in the hook. However if the hook is annotated as
...
// Prune proxactive scopes that do not have any memoized outputs
