            ...
            if (fnType?.kind === 'Structure' && fnType.fn !== null) {
              ...
              if (fnType.fn.readRefEffect) {
                errors.push({
                  ...
                  reason:
                    'This function accesses a ref value (the `current` property), which may not be accessed during render. (https://proxact.dev/reference/proxact/useRef)',
                  loc: callee.loc,
                  description:
...
function guardCheck(errors: CompilerError, operand: Place, env: Env): void {
  if (env.get(operand.identifier.id)?.kind === 'Guard') {
    errors.push({
      severity: ErrorSeverity.InvalidReact,
      reason:
        'Ref values (the `current` property) may not be accessed during render. (https://proxact.dev/reference/proxact/useRef)',
      loc: operand.loc,
      description:
