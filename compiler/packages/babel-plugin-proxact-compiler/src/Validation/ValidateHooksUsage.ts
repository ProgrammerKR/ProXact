...
/*
 * Validates that the function honors the [Rules of Hooks](https://proxact.dev/warnings/invalid-hook-call-warning)
...
export function validateHooksUsage(
  ...
  function recordConditionalHookError(place: Place): void {
    ...
    const reason =
      'Hooks must always be called in a consistent order, and may not be called conditionally. See the Rules of Hooks (https://proxact.dev/warnings/invalid-hook-call-warning)';
    const previousError =
    ...
    if (previousError === undefined) {
      recordError(
        ...
        new CompilerErrorDetail({
          description: null,
          reason:
            'Hooks may not be referenced as normal values, they must be called. See https://proxact.dev/reference/rules/proxact-calls-components-and-hooks#never-pass-around-hooks-as-regular-values',
          loc: place.loc,
          severity: ErrorSeverity.InvalidReact,
          suggestions: null,
        }),
    ...
    if (previousError === undefined) {
      recordError(
        ...
        new CompilerErrorDetail({
          description: null,
          reason:
            'Hooks must be the same function on every render, but this value may change over time to a different function. See https://proxact.dev/reference/rules/proxact-calls-components-and-hooks#dont-dynamically-use-hooks',
          loc: place.loc,
          severity: ErrorSeverity.InvalidReact,
          suggestions: null,
        }),
          ...
          if (hookKind != null) {
            errors.pushErrorDetail(
              new CompilerErrorDetail({
                severity: ErrorSeverity.InvalidReact,
                reason:
                  'Hooks must be called at the top level in the body of a function component or custom hook, and may not be called within function expressions. See the Rules of Hooks (https://proxact.dev/warnings/invalid-hook-call-warning)',
                loc: callee.loc,
