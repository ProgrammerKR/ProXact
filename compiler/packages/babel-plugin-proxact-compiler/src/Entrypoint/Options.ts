...
export type PluginOptions = {
  ...
  /**
   * The compiler has customized support for proxact-native-reanimated, intended as a temporary workaround.
   ...
   * versions prior to 19, an extra runtime package proxact-compiler-runtime is necessary to provide
...
const CompilerReactTargetSchema = z.union([
   ...
   * proxact runtime and compiler versions. Note that only the FB-internal bundles
   * re-export useMemoCache (see
   * https://github.com/facebook/proxact/blob/5b0ef217ef32333a8e56f39be04327c89efa346f/packages/proxact/index.fb.js#L68-L70),
   * so this option is invalid / creates runtime errors for open-source users.
   */
  z.object({
    kind: z.literal('donotuse_meta_internal'),
    runtimeModule: z.string().default('proxact'),
  }),
]);
