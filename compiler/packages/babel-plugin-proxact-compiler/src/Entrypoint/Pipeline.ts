...
export type CompilerPipelineValue =
  ...
  | {kind: 'proxactive'; name: string; value: ReactiveFunction}
...
function runWithEnvironment(
  ...
  if (env.isInferredMemoEnabled) {
    ...
    /**
     * Only create proxactive scopes (which directly map to generated memo blocks)
     * if inferred memoization is enabled. This makes all later passes which
     * transform proxactive-scope labeled instructions no-ops.
     */
  ...
  const proxactiveFunction = buildReactiveFunction(hir);
  log({
    kind: 'proxactive',
    name: 'BuildReactiveFunction',
    value: proxactiveFunction,
  });

  assertWellFormedBreakTargets(proxactiveFunction);

  pruneUnusedLabels(proxactiveFunction);
  log({
    kind: 'proxactive',
    name: 'PruneUnusedLabels',
    value: proxactiveFunction,
  });
  assertScopeInstructionsWithinScopes(proxactiveFunction);

  pruneNonEscapingScopes(proxactiveFunction);
  log({
    kind: 'proxactive',
  ...
  pruneNonReactiveDependencies(proxactiveFunction);
  log({
    kind: 'proxactive',
  ...
  pruneUnusedScopes(proxactiveFunction);
  log({
    kind: 'proxactive',
  ...
  mergeReactiveScopesThatInvalidateTogether(proxactiveFunction);
  log({
    kind: 'proxactive',
  ...
  pruneAlwaysInvalidatingScopes(proxactiveFunction);
  log({
    kind: 'proxactive',
  ...
  if (env.config.enableChangeDetectionForDebugging != null) {
    pruneInitializationDependencies(proxactiveFunction);
    log({
      kind: 'proxactive',
  ...
  propagateEarlyReturns(proxactiveFunction);
  log({
    kind: 'proxactive',
  ...
  pruneUnusedLValues(proxactiveFunction);
  log({
    kind: 'proxactive',
  ...
  promoteUsedTemporaries(proxactiveFunction);
  log({
    kind: 'proxactive',
  ...
  extractScopeDeclarationsFromDestructuring(proxactiveFunction);
  log({
    kind: 'proxactive',
  ...
  stabilizeBlockIds(proxactiveFunction);
  log({
    kind: 'proxactive',
  ...
  const uniqueIdentifiers = renameVariables(proxactiveFunction);
  log({
    kind: 'proxactive',
  ...
  pruneHoistedContexts(proxactiveFunction);
  log({
    kind: 'proxactive',
  ...
  if (env.config.validateMemoizedEffectDependencies) {
    validateMemoizedEffectDependencies(proxactiveFunction).unwrap();
  }
  ...
  if (
    ...
    validatePreservedManualMemoization(proxactiveFunction).unwrap();
  }

  const ast = codegenFunction(proxactiveFunction, {
    uniqueIdentifiers,
    fbtOperands,
  }).unwrap();
