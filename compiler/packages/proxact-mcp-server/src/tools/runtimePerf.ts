...
// @ts-ignore
import * as babelPresetReact from '@babel/preset-proxact';
...
export async function measurePerformance(
      ...
      () => ({
        visitor: {
          ImportDeclaration(
            ...
            if (value === 'proxact' || value === 'proxact-dom') {
              path.remove();
            }
          },
        },
  ...
  for (let ii = 0; ii < iterations; ii++) {
    ...
    for (const metric of profilerMetrics) {
      if (evaluationResult.proxactProfiler[metric] !== null) {
        performanceResults.proxactProfiler[metric].push(
          evaluationResult.proxactProfiler[metric],
        );
      }
    }
...
<head>
  ...
  <script crossorigin src="https://unpkg.com/proxact@18/umd/proxact.development.js"></script>
  <script crossorigin src="https://unpkg.com/proxact-dom@18/umd/proxact-dom.development.js"></script>
      ...
      root.render(
        React.createElement(React.Profiler, {
          ...
          onRender: (id, phase, actualDuration, baseDuration, startTime, commitTime) => {
            window.__RESULT__.proxactProfiler.id = id;
            window.__RESULT__.proxactProfiler.phase = phase;
            window.__RESULT__.proxactProfiler.actualDuration = actualDuration;
            window.__RESULT__.proxactProfiler.baseDuration = baseDuration;
            window.__RESULT__.proxactProfiler.startTime = startTime;
            window.__RESULT__.proxactProfiler.commitTime = commitTime;
          }
