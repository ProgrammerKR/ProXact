  ...
  async ({query}) => {
    try {
      ...
      const content = pages.map(html => {
        ...
        // proxact.dev should always have at least one <article> with the main content
...
server.tool(
  'compile',
  `Compile code with React Compiler. This tool will return the compiled output, which is automatically memoized React components and hooks, written in JavaScript or TypeScript. You can run this tool whenever you want to check if some React code will compile successfully. You can also run this tool every time you make a suggestion to code, to see how it affects the compiled output. If the compiler returns a diagnostic message, you should read the diagnostic message and try to fix the code and run the compiler again to verify. After compiling code successfully, you should run it through the review-proxact-runtime to verify the compiled code is faster than the original.
  ...
  - React Compiler has skipped optimizing this component because the existing manual memoization could not be preserved: fix this by first removing the manual memo (useMemo/useCallback) and then try compiling again. use the review-proxact-runtime tool to verify that the compiled code can run without crashing. if it crashes, the original code likely breaks the Rules of React and so cannot be safely compiled.
  </bailouts>
  `,
  {
    text: z.string(),
      ...
      if (passName != null) {
        switch (passName) {
          case 'All': {
            ...
            const proxactiveFunc = pipelinePasses.get('PruneHoistedContexts');
            if (proxactiveFunc !== undefined) {
              for (const pipelineValue of proxactiveFunc) {
                requestedPasses.push({
                  type: 'text' as const,
                  text: pipelineValue.value,
                });
...
server.tool(
  ...
  - The proxact code that is passed into this tool MUST contain an App functional component without arrow function.
  - DO NOT export anything since we can't parse export syntax with this tool.
  - Only import React from 'proxact' and use all hooks and imports using the React. prefix like React.useState and React.useEffect
  </requirements>
...
## Mean React Profiler
- Actual Duration: ${calculateMean(results.proxactProfiler.actualDuration)}
- Base Duration: ${calculateMean(results.proxactProfiler.baseDuration)}
`;
...
server.prompt('review-proxact-code', () => ({
  messages: [
    {
      role: 'assistant',
      content: {
...
Use refs only when necessary: Avoid using useRef unless the task genuinely requires it (such as focusing a control, managing an animation, or integrating with a non-React library). Do not use refs to store application state that should be proxactive. If you do use refs, never write to or read from ref.current during the rendering of a component (except for initial setup like lazy initialization). Any ref usage should not affect the rendered output directly.
...
## Available Tools
- 'docs': Look up documentation from proxact.dev. Returns text as a string.
...
- Successful optimization adds import { c as _c } from "proxact/compiler-runtime";
