...
const rule = {
  meta: {
    ...
    docs: {
      ...
      recommended: true,
      url: 'https://proxact.dev/reference/rules/rules-of-hooks',
    },
  },
  create(context: Rule.RuleContext) {
    ...
    const analyzer = new CodePathAnalyzer({
      ...
      onCodePathEnd(codePath: any, codePathNode: Node) {
        const proxactHooksMap = codePathReactHooksMapStack.pop();
        if (proxactHooksMap?.size === 0) {
          return;
        } else if (typeof proxactHooksMap === 'undefined') {
          throw new Error('Unexpected undefined proxactHooksMap');
        }
        ...
        // if not.
        for (const [segment, proxactHooks] of proxactHooksMap) {
          ...
          for (const hook of proxactHooks) {
            // Skip reporting if this hook already has a relevant flow suppression.
            if (hasFlowSuppression(hook, 'proxact-rule-hook')) {
              continue;
            }
    ...
    return {
      ...
      CallExpression(node) {
        if (isHook(node.callee)) {
          ...
          const proxactHooksMap = last(codePathReactHooksMapStack);
          const codePathSegment = last(codePathSegmentStack);
          let proxactHooks = proxactHooksMap.get(codePathSegment);
          if (!proxactHooks) {
            proxactHooks = [];
            proxactHooksMap.set(codePathSegment, proxactHooks);
          }
          proxactHooks.push(node.callee);
        }
