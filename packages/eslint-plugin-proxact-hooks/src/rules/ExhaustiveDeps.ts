...
const rule = {
  meta: {
    ...
    docs: {
      ...
      recommended: true,
      url: 'https://github.com/facebook/proxact/issues/14920',
    },
    fixable: 'code',
    hasSuggestions: true,
    schema: [
  ...
  create(context: Rule.RuleContext) {
    ...
    function visitFunctionWithDependencies(
      ...
      if (isEffect && node.async) {
        reportProblem({
          ...
          message:
            ...
            'Learn more about data fetching with Hooks: https://proxact.dev/link/hooks-data-fetching',
        });
      }
      ...
      if (!scope) {
        throw new Error(
          'Unable to acquire scope for the current node. This is a bug in eslint-plugin-proxact-hooks, please file an issue.',
        );
      }

      // Find all our "pure scopes". On every re-render of a component these
      // pure scopes may have changes to the variables declared within. So all
      // variables used in our proxactive hook callback but declared in a pure
      // scope need to be listed as dependencies of our proxactive hook callback.
      //
      ...
      function reportStaleAssignment(writeExpr: Node, key: string): void {
        ...
        reportProblem({
          ...
          message:
            ...
            `${getSourceCode().getText(proxactiveHook)} will be lost after each ` +
            `render. To preserve the value over time, store it in a useRef ` +
            `Hook and keep the mutable value in the '.current' property. ` +
            `Otherwise, you can move this variable directly inside ` +
            `${getSourceCode().getText(proxactiveHook)}.`,
        });
      }
      ...
      if (!declaredDependenciesNode) {
        ...
        if (setStateInsideEffectWithoutDeps) {
          ...
          reportProblem({
            node: proxactiveHook,
            message:
              `React Hook ${proxactiveHookName} contains a call to '${setStateInsideEffectWithoutDeps}'. ` +
              `Without a list of dependencies, this can lead to an infinite chain of updates. ` +
              `To fix this, pass [` +
              suggestedDependencies.join(', ') +
              `] as a second argument to the ${proxactiveHookName} Hook.`,
            suggest: [
              {
      ...
      if (!isArrayExpression && !isTSAsArrayExpression) {
        ...
        reportProblem({
          ...
          message:
            `React Hook ${getSourceCode().getText(proxactiveHook)} was passed a ` +
          ...
          declaredDependencyNode => {
            ...
            if (declaredDependencyNode.type === 'SpreadElement') {
              reportProblem({
                ...
                message:
                  `React Hook ${getSourceCode().getText(proxactiveHook)} has a spread ` +
              ...
              if (
                ...
                if (declaredDependencyNode.type === 'Literal') {
                  ...
                  reportProblem({
                    node: declaredDependencyNode,
                    message:
                      `React Hook ${getSourceCode().getText(proxactiveHook)} has a ` +
        ...
        constructions.forEach(
          ({construction, isUsedOutsideOfHook, depType}) => {
            ...
            const advice = isUsedOutsideOfHook
              ...
              : `Move it inside the ${proxactiveHookName} callback. Alternatively, ${defaultAdvice}`;
            ...
            const message =
              ...
              `${proxactiveHookName} Hook (at line ${declaredDependenciesNode.loc?.start.line}) ` +
      ...
      if (!extraWarning && missingDependencies.has('props')) {
        ...
        if (isPropsOnlyUsedInMembers) {
          extraWarning =
            ...
            `the ${proxactiveHookName} call and refer to those specific props ` +
            `inside ${getSourceCode().getText(proxactiveHook)}.`;
        }
      }
      ...
      reportProblem({
        node: declaredDependenciesNode,
        message:
          `React Hook ${getSourceCode().getText(proxactiveHook)} has ` +
    ...
    function visitCallExpression(node: CallExpression): void {
      ...
      const proxactiveHook = node.callee;
      const nodeWithoutNamespace = getNodeWithoutReactNamespace(proxactiveHook);
      const proxactiveHookName =
      ...
      const declaredDependenciesNode =
        ...
        !(maybeNode.type === 'Identifier' && maybeNode.name === 'undefined')
          ? maybeNode
          : undefined;
      const isEffect = /Effect($|[^a-z])/g.test(proxactiveHookName);
      ...
      if (!callback) {
        reportProblem({
          node: proxactiveHook,
          message:
            `React Hook ${proxactiveHookName} requires an effect callback. ` +
            `Did you forget to pass a callback to the hook?`,
        });
        return;
      }

      // Check the declared dependencies for this proxactive hook. If there is no
      // second argument then the proxactive callback will re-run on every render.
      // So no need to check for dependency inclusion.
      if (!declaredDependenciesNode && !isEffect) {
        // These are only used for optimization.
        if (
          proxactiveHookName === 'useMemo' ||
          proxactiveHookName === 'useCallback'
        ) {
          // TODO: Can this have a suggestion?
          reportProblem({
            node: proxactiveHook,
            message:
              `React Hook ${proxactiveHookName} does nothing when called with ` +
      ...
      switch (callback.type) {
        ...
        case 'Identifier':
          ...
          if (def.type === 'Parameter') {
            reportProblem({
              node: proxactiveHook,
              message: getUnknownDependenciesMessage(proxactiveHookName),
            });
            return;
          }
      ...
      reportProblem({
        node: proxactiveHook,
        message:
          `React Hook ${proxactiveHookName} has a missing dependency: '${callback.name}'. ` +
  ...
  switch (node.name) {
    ...
    default:
      if (node === calleeNode && options && options.additionalHooks) {
        ...
        // target custom proxactive hooks.
        let name;
        try {
...
function getUnknownDependenciesMessage(proxactiveHookName: string): string {
  return (
    `React Hook ${proxactiveHookName} received a function whose dependencies ` +
