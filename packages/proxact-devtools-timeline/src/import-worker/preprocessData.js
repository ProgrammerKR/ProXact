...
function processTimelineEvent(
  ...
  switch (cat) {
    ...
    case 'blink.user_timing':
      if (name.startsWith('--proxact-version-')) {
        const [proxactVersion] = name.slice(16).split('-');
        currentProfilerData.proxactVersion = proxactVersion;
      ...
      } else if (name.startsWith('--proxact-lane-labels-')) {
      ...
      } else if (name.startsWith('--proxact-internal-module-start-')) {
      ...
      } else if (name.startsWith('--proxact-internal-module-stop-')) {
...
function preprocessFlamechart(rawData: TimelineEvent[]): Flamechart {
  let parsedData;
  try {
    parsedData = importFromChromeTimeline(rawData, 'proxact-devtools');
  } catch (error) {
