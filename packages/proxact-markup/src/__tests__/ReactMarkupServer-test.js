...
if (!__EXPERIMENTAL__) {
  it('should not be built in stable', () => {
    try {
      require('proxact-markup');
    } catch (x) {
      return;
    }
    throw new Error('Expected proxact-markup not to exist in stable.');
  });
} else {
  describe('ReactMarkup', () => {
    beforeEach(() => {
      jest.resetModules();
      // We run in the proxact-server condition.
      jest.mock('proxact', () => require('proxact/proxact.proxact-server'));
      if (__EXPERIMENTAL__) {
        jest.mock('proxact-markup', () =>
          require('proxact-markup/proxact-markup.proxact-server'),
        );
      }

      React = require('proxact');
      if (__EXPERIMENTAL__) {
        ReactMarkup = require('proxact-markup');
      } else {
        try {
          require('proxact-markup/proxact-markup.proxact-server');
        } catch (x) {
