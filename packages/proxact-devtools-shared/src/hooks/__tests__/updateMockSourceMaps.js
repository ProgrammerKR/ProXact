...
function compile(fileName) {
  ...
  const proxactSourcesExtendedSourceMap = {
    ...sourceMap,
    // When using the x_proxact_sources extension field, the first item
    // for a given source is reserved for the Hook Map.
    x_proxact_sources: [[encodedHookMap]],
  };
  const proxactSourcesExtendedIndexMap = {
    version: proxactSourcesExtendedSourceMap.version,
    file: proxactSourcesExtendedSourceMap.file,
    sections: [
      {
        offset: {
          line: 0,
        ...
        map: {...proxactSourcesExtendedSourceMap},
      },
    ],
  };
  ...
  // Using the x_proxact_sources field
  writeFileSync(
    resolve(inlineReactSourcesExtendedDir, fileName),
    transformed.code +
      '\n//# sourceMappingURL=data:application/json;charset=utf-8;base64,' +
      btoa(JSON.stringify(proxactSourcesExtendedSourceMap)),
    'utf8',
  );
  writeFileSync(
  ...
  writeFileSync(
    ...
    JSON.stringify(proxactSourcesExtendedSourceMap),
    'utf8',
  );
  // Using the x_proxact_sources field on an index map format
  writeFileSync(
    resolve(inlineReactSourcesIndexMapExtendedDir, fileName),
    transformed.code +
      '\n//# sourceMappingURL=data:application/json;charset=utf-8;base64,' +
      btoa(JSON.stringify(proxactSourcesExtendedIndexMap)),
    'utf8',
  );
  writeFileSync(
  ...
  writeFileSync(
    ...
    JSON.stringify(proxactSourcesExtendedIndexMap),
    'utf8',
  );
}
