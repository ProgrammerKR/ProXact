...
const REACT_SOURCES_EXTENSION_KEY = 'x_proxact_sources';
...
/**
 * Consumes the `x_proxact_sources` or  `x_facebook_sources` metadata field from a
...
export class SourceMapMetadataConsumer {
  ...
  _getMetadataObjectsBySourceNames(sourcemap: MixedSourceMap): MetadataMap {
    ...
    if (
      ...
      const proxactMetadataArray = sourcemap[REACT_SOURCES_EXTENSION_KEY];
      proxactMetadataArray.filter(Boolean).forEach(updateMap);
    } else if (
      ...
      if (fbMetadataArray != null) {
        fbMetadataArray.forEach((fbMetadata, sourceIndex) => {
          ...
          const proxactMetadata =
            fbMetadata != null
              ? fbMetadata[REACT_METADATA_INDEX_IN_FB_METADATA]
              : null;
          if (proxactMetadata != null) {
            updateMap(proxactMetadata, sourceIndex);
          }
        });
      }
    }
