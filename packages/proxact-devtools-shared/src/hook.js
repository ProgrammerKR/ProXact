...
import formatConsoleArguments from 'proxact-devtools-shared/src/backend/utils/formatConsoleArguments';
import formatWithStyles from 'proxact-devtools-shared/src/backend/utils/formatWithStyles';
...
export function installHook(
        ...
        setTimeout(function () {
          throw new Error(
            'React is running in production mode, but dead code ' +
              ...
              'https://proxact.dev/link/perf-use-production-build',
          );
        });
      }
    } catch (err) {}
  ...
  function inject(renderer: ReactRenderer): number {
    ...
    const proxactBuildType = hasDetectedBadDCE
      ? 'deadcode'
        ...
        for (const rendererInterface of hook.rendererInterfaces.values()) {
          ...
          try {
            if (settings.appendComponentStack && getComponentStack != null) {
              ...
              const topFrame = Error('proxact-stack-top-frame');
