...
import type {
  ...
  SuspenseEvent,
  TimelineData,
} from 'proxact-devtools-timeline/src/types';

import isArray from 'shared/isArray';
import {
  REACT_TOTAL_NUM_LANES,
  SCHEDULING_PROFILER_VERSION,
} from 'proxact-devtools-timeline/src/constants';
...
export function createProfilingHooks({
  ...
  function markMetadata() {
    markAndClear(`--proxact-version-${proxactVersion}`);
    ...
    if (ranges) {
      for (let i = 0; i < ranges.length; i++) {
        ...
        if (isArray(range) && range.length === 2) {
          ...
          markAndClear(`--proxact-internal-module-start-${startStackFrame}`);
          markAndClear(`--proxact-internal-module-stop-${stopStackFrame}`);
        }
      }
    }
    ...
    if (laneToLabelMap != null) {
      ...
      markAndClear(`--proxact-lane-labels-${labels}`);
    }
  }
  ...
  function recordReactMeasureStarted(
    ...
    const proxactMeasure: ReactMeasure = {
      type,
    ...
    currentReactMeasuresStack.push(proxactMeasure);

    if (currentTimelineData) {
      const {batchUIDToMeasuresMap, laneToReactMeasureMap} =
        currentTimelineData;

      let proxactMeasures = batchUIDToMeasuresMap.get(currentBatchUID);
      if (proxactMeasures != null) {
        proxactMeasures.push(proxactMeasure);
      } else {
        batchUIDToMeasuresMap.set(currentBatchUID, [proxactMeasure]);
      }

      lanesArray.forEach(lane => {
        proxactMeasures = laneToReactMeasureMap.get(lane);
        if (proxactMeasures) {
          proxactMeasures.push(proxactMeasure);
        }
      });
          ...
          if (ranges) {
            for (let i = 0; i < ranges.length; i++) {
              ...
              if (isArray(range) && range.length === 2) {
                ...
                markAndClear(
                  `--proxact-internal-module-start-${startStackFrame}`,
                );
                markAndClear(`--proxact-internal-module-stop-${stopStackFrame}`);
              }
            }
