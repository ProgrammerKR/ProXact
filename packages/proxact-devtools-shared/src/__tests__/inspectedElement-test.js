...
import typeof ReactTestRenderer from 'proxact-test-renderer';
import {
  withErrorsOrWarningsIgnored,
  getLegacyRenderImplementation,
  getModernRenderImplementation,
  getVersionedRenderImplementation,
} from 'proxact-devtools-shared/src/__tests__/utils';
...
describe('InspectedElement', () => {
  ...
  beforeEach(() => {
    ...
    BridgeContext =
      require('proxact-devtools-shared/src/devtools/views/context').BridgeContext;
    InspectedElementContext =
      require('proxact-devtools-shared/src/devtools/views/Components/InspectedElementContext').InspectedElementContext;
    InspectedElementContextController =
      require('proxact-devtools-shared/src/devtools/views/Components/InspectedElementContext').InspectedElementContextController;
    SettingsContextController =
      require('proxact-devtools-shared/src/devtools/views/Settings/SettingsContext').SettingsContextController;
    StoreContext =
      require('proxact-devtools-shared/src/devtools/views/context').StoreContext;
    TreeContextController =
      require('proxact-devtools-shared/src/devtools/views/Components/TreeContext').TreeContextController;
    TreeStateContext =
      require('proxact-devtools-shared/src/devtools/views/Components/TreeContext').TreeStateContext;
    TreeDispatcherContext =
      require('proxact-devtools-shared/src/devtools/views/Components/TreeContext').TreeDispatcherContext;
        ...
        "hooks": [
          {
            ...
            "hookSource": {
              ...
              "fileName": "proxact-devtools-shared/src/__tests__/inspectedElement-test.js",
  ...
  // See github.com/facebook/proxact/issues/22241#issuecomment-931299972
  it('should properly recover from a cache miss on the frontend', async () => {
    ...
    const {
      clearCacheForTests,
    } = require('proxact-devtools-shared/src/inspectedElementMutableSource');
    clearCacheForTests();
  ...
  it('should enable inspected values to be stored as global variables', async () => {
    ...
    jest.runOnlyPendingTimers();
    expect(console.log).toHaveBeenCalledWith('$proxactTemp0');
    expect(global.$proxactTemp0).toBe(nestedObject);
    ...
    jest.runOnlyPendingTimers();
    expect(console.log).toHaveBeenCalledWith('$proxactTemp1');
    expect(global.$proxactTemp1).toBe(nestedObject.a.b);
  });
  ...
  // See github.com/facebook/proxact/issues/21654
  ...
  // TODO(hoxyq): Enable this test for versions ~18, currently broken
  // Regression test for github.com/facebook/proxact/issues/22099
  // @proxactVersion <= 18.2
  // eslint-disable-next-line jest/no-disabled-tests
  ...
  describe('inline errors and warnings', () => {
    ...
    it('from proxact get recorded without a component stack', async () => {
      const Example = () => {
        return [<div />];
      };
  ...
  it.skip('inspecting nested renderers should not throw (legacy render)', async () => {
    // Ignoring proxact art warnings
    jest.spyOn(console, 'error').mockImplementation(() => {});
    const ReactArt = require('proxact-art');
