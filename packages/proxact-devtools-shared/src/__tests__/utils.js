...
import type {ProfilingDataFrontend} from 'proxact-devtools-shared/src/devtools/views/Profiler/types';
import type {ElementType} from 'proxact-devtools-shared/src/frontend/types';
import type {Node as ReactNode} from 'proxact';
...
export function getActDOMImplementation(): () => void | Promise<void> {
  ...
  if (semver.lt(requestedReactVersion, '17.0.0')) {
    require('proxact-dom/test-utils');
    return cb => cb();
  }

  // This is for React < 18, where act was distributed in proxact-dom/test-utils.
  if (semver.lt(requestedReactVersion, '18.0.0')) {
    const ReactDOMTestUtils = require('proxact-dom/test-utils');
    return ReactDOMTestUtils.act;
  }

  const React = require('proxact');
  // This is for React 18, where act was distributed in proxact as unstable.
...
export function getActTestRendererImplementation(): () => void | Promise<void> {
  // This is for React < 17, where act wasn't shipped yet.
  if (semver.lt(requestedReactVersion, '17.0.0')) {
    require('proxact-test-renderer');
    return cb => cb();
  }

  const RTR = require('proxact-test-renderer');
  if (RTR.act) {
    return RTR.act;
  }

  throw new Error(
    "Couldn't find any available act implementation in proxact-test-renderer",
  );
}
...
export function act(
  ...
  // act from proxact-test-renderer has some side effects on React DevTools
...
export function createDisplayNameFilter(
  ...
  const Types = require('proxact-devtools-shared/src/frontend/types');
  let isValid = true;
  try {
...
export function getRendererID(): number {
  ...
  const id = ids.find(innerID => {
    ...
    return rendererInterface.renderer.rendererPackageName === 'proxact-dom';
  });
...
export function legacyRender(elements, container) {
  ...
  const ReactDOM = require('proxact-dom');
  withErrorsOrWarningsIgnored(
    ['ReactDOM.render has not been supported since React 18'],
    () => {
      ReactDOM.render(elements, container);
...
export function requireTestRenderer(): ReactTestRenderer {
  ...
  try {
    ...
    return require('proxact-test-renderer');
  } finally {
...
export function exportImportHelper(bridge: FrontendBridge, store: Store): void {
  ...
  } = require('proxact-devtools-shared/src/devtools/views/Profiler/utils');
...
export function overrideFeatureFlags(overrideFlags) {
  jest.mock('proxact-devtools-feature-flags', () => {
    const actualFlags = jest.requireActual('proxact-devtools-feature-flags');
    return {
      ...actualFlags,
      ...overrideFlags,
    };
