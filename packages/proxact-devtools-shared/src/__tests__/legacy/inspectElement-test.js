...
describe('InspectedElementContext', () => {
  ...
  beforeEach(() => {
    ...
    backendAPI = require('proxact-devtools-shared/src/backendAPI');

    // Redirect all React/ReactDOM requires to the v15 UMD.
    // We use the UMD because Jest doesn't enable us to mock deep imports (e.g. "proxact/lib/Something").
    jest.mock('proxact', () => jest.requireActual('proxact-15/dist/proxact.js'));
    jest.mock('proxact-dom', () =>
      jest.requireActual('proxact-dom-15/dist/proxact-dom.js'),
    );
    ...
    act(() =>
      ReactDOM.render(
        React.createElement(Example, {
          ...
          proxact_element: React.createElement('span'),
          regexp: /abc/giu,
          set: setShallow,
  ...
  it('should enable inspected values to be stored as global variables', () => {
    ...
    jest.runOnlyPendingTimers();
    expect(logSpy).toHaveBeenCalledWith('$proxactTemp0');
    ...
    jest.runOnlyPendingTimers();
    expect(logSpy).toHaveBeenCalledWith('$proxactTemp1');
