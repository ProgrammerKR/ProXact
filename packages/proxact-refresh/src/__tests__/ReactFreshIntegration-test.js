...
const freshPlugin = require('proxact-refresh/babel');
...
describe('ReactFreshIntegration', () => {
  ...
  function testJavaScript(render, patch) {
    ...
    it('ignores ref for class component in hidden subtree', async () => {
      const code = `
        import {unstable_Activity as Activity} from 'proxact';
    ...
    it('resets useMemoCache cache slots', async () => {
      if (__DEV__) {
        await render(`
          const useMemoCache = require('proxact/compiler-runtime').c;
          let cacheMisses = 0;
          const cacheMiss = (id) => {
            cacheMisses++;
            return id;
