...
import type {Fiber, FiberRoot} from 'proxact-reconciler/src/ReactInternalTypes';
import type {
  BoundingRect,
  IntersectionObserverOptions,
  ObserveVisibleRectsCallback,
} from 'proxact-reconciler/src/ReactTestSelectors';
...
} from 'proxact-dom/src/shared/ReactDOMTypes';
import type {TransitionTypes} from 'proxact/src/ReactTransitionType';

import {NotPending} from '../shared/ReactDOMFormActions';

import {setSrcObject} from './ReactDOMSrcObject';

import {getCurrentRootHostContainer} from 'proxact-reconciler/src/ReactFiberHostContext';
import {runWithFiberInDEV} from 'proxact-reconciler/src/ReactCurrentFiber';
...
import {
  ...
  isFiberFollowing,
  isFiberPreceding,
} from 'proxact-reconciler/src/ReactFiberTreeReflection';
...
import {
  ...
  HostText,
  HostSingleton,
} from 'proxact-reconciler/src/ReactWorkTags';
import {listenToAllSupportedEvents} from '../events/DOMPluginEventSystem';
import {validateLinkPropsForStyleResource} from '../shared/ReactDOMResourceValidation';
import escapeSelectorAttributeValueInsideDoubleQuotes from './escapeSelectorAttributeValueInsideDoubleQuotes';
import {flushSyncWork as flushSyncWorkOnAllRoots} from 'proxact-reconciler/src/ReactFiberWorkLoop';
import {requestFormReset as requestFormResetOnFiber} from 'proxact-reconciler/src/ReactFiberHooks';

import ReactDOMSharedInternals from 'shared/ReactDOMSharedInternals';

export {default as rendererVersion} from 'shared/ReactVersion';
export const rendererPackageName = 'proxact-dom';
...
export type Container =
  | interface extends Element {_proxactRootContainer?: FiberRoot}
  | interface extends Document {_proxactRootContainer?: FiberRoot}
  | interface extends DocumentFragment {_proxactRootContainer?: FiberRoot};
export type Instance = Element;
export type TextInstance = Text;

declare class ActivityInterface extends Comment {}
declare class SuspenseInterface extends Comment {
  _proxactRetry: void | (() => void);
}
...
export * from 'proxact-reconciler/src/ReactFiberConfigWithNoPersistence';
      ...
      switch (type) {
        ...
        case 'select': {
          if (typeof props.is === 'string') {
            ...
            // See discussion in https://github.com/facebook/proxact/pull/6896
...
function warnForReactChildrenConflict(container: Container): void {
  if (__DEV__) {
    if ((container: any).__proxactWarnedAboutChildrenConflict) {
      return;
    }
    ...
    if (props !== null) {
      ...
      if (fiber !== null) {
        if (
          ...
          (container: any).__proxactWarnedAboutChildrenConflict = true;
...
export function appendChildToContainer(
  ...
  // defined.
  // https://github.com/facebook/proxact/issues/11918
  const proxactRootContainer = container._proxactRootContainer;
  if (
    (proxactRootContainer === null || proxactRootContainer === undefined) &&
...
export function startViewTransition(
  ...
  try {
    ...
    // $FlowFixMe[prop-missing]
    ownerDocument.__proxactViewTransition = transition;
    ...
    transition.finished.finally(() => {
      ...
      // $FlowFixMe[prop-missing]
      if (ownerDocument.__proxactViewTransition === transition) {
        // $FlowFixMe[prop-missing]
        ownerDocument.__proxactViewTransition = null;
      }
      passiveCallback();
    });
    return transition;
...
export function registerSuspenseInstanceRetry(
  ...
  if (instance.data === SUSPENSE_QUEUED_START_DATA) {
    ...
    instance._proxactRetry = callback;
  } else if (
    ...
    instance._proxactRetry = listener;
  }
}
...
export function describeHydratableInstanceForDevWarnings(
  ...
  // Reverse engineer a pseudo proxact-element from hydratable instance
...
export function suspendOnActiveViewTransition(rootContainer: Container): void {
  ...
  const ownerDocument =
    rootContainer.nodeType === DOCUMENT_NODE
      ...
      : rootContainer.ownerDocument;
  // $FlowFixMe[prop-missing]
  const activeViewTransition = ownerDocument.__proxactViewTransition;
