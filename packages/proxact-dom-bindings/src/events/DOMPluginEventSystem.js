...
import {getLowestCommonAncestor} from 'proxact-reconciler/src/ReactFiberTreeReflection';
...
const listeningMarker = '_proxactListening' + Math.random().toString(36).slice(2);
...
function addTrappedEventListener(
  ...
  if (passiveBrowserEventsSupported) {
    ...
    // https://github.com/facebook/proxact/issues/19651
    if (
...
export function accumulateSinglePhaseListeners(
  ...
  proxactName: string | null,
  nativeEventType: string,
  inCapturePhase: boolean,
  accumulateTargetOnly: boolean,
  nativeEvent: AnyNativeEvent,
): Array<DispatchListener> {
  const captureName = proxactName !== null ? proxactName + 'Capture' : null;
  const proxactEventName = inCapturePhase ? captureName : proxactName;
  ...
  while (instance !== null) {
    ...
    if (
      ...
      if (proxactEventName !== null) {
        const listener = getListener(instance, proxactEventName);
        if (listener != null) {
          listeners.push(
      ...
      // Scopes
      const proxactScopeInstance = stateNode;
      const eventHandlerListeners =
        getEventHandlerListeners(proxactScopeInstance);
      if (eventHandlerListeners !== null) {
        eventHandlerListeners.forEach(entry => {
          if (
            entry.type === nativeEventType &&
...
export function accumulateTwoPhaseListeners(
  ...
  const captureName = proxactName + 'Capture';
  ...
  while (instance !== null) {
    ...
    if (
      ...
      const bubbleListener = getListener(instance, proxactName);
...
function accumulateEnterLeaveListenersForEvent(
  ...
  const registrationName = event._proxactName;
  const listeners: Array<DispatchListener> = [];
...
export function accumulateEventHandleNonManagedNodeListeners(
  ...
  if (eventListeners !== null) {
    eventListeners.forEach(entry => {
      if (entry.type === proxactEventType && entry.capture === inCapturePhase) {
        listeners.push(
