...
export function handleReactDevToolsHookMessage(message, sender) {
  ...
  switch (payload?.type) {
    case 'proxact-renderer-attached': {
      setExtensionIconAndPopup(payload.proxactBuildType, sender.tab.id);
      ...
      payload.versions.forEach(version => {
        if (EXTENSION_CONTAINED_VERSIONS.includes(version)) {
          executeScriptInMainWorld({
            ...
            files: [`/build/proxact_devtools_backend_${version}.js`],
          });
        }
      });
  ...
  switch (payload?.type) {
    ...
    case 'fetch-file-with-cache-error':
      ...
      chrome.runtime.sendMessage({
        source: 'proxact-devtools-background',
        payload,
      });
      break;
  }
