...
import {COMPACT_VERSION_NAME} from 'proxact-devtools-extensions/src/utils';
import {getIsReloadAndProfileSupported} from 'proxact-devtools-shared/src/utils';
import {
...
function welcome(event: $FlowFixMe) {
  if (
    event.source !== window ||
    event.data.source !== 'proxact-devtools-content-script'
  ) {
    return;
  }
  ...
  //
  // See https://github.com/facebook/proxact/issues/24162
...
function activateBackend(version: string, hook: DevToolsHook) {
  ...
  const bridge = new Bridge({
    ...
    send(event: string, payload: any, transferable?: Array<any>) {
      window.postMessage(
        {
          source: 'proxact-devtools-bridge',
  ...
  // Setup React Native style editor if a renderer like proxact-native-web has injected it.
...
function updateRequiredBackends() {
  ...
  window.postMessage(
    {
      source: 'proxact-devtools-backend-manager',
      payload: {
 ...
 * We can't use `proxactDevToolsAgent` field on a global Hook object, because it only cleaned up after both Frontend and Backend initialized
 */
