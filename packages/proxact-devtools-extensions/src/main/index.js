...
import {startReactPolling} from './proxactPolling';
...
function createBridgeAndStore() {
  ...
  const hookNamesModuleLoaderFunction = () =>
    import(
      /* webpackChunkName: 'parseHookNames' */ 'proxact-devtools-shared/src/hooks/parseHookNames'
    );
...
function performInTabNavigationCleanup() {
  // Potentially, if proxact hasn't loaded yet and user performs in-tab navigation
...
function performFullCleanup() {
  // Potentially, if proxact hasn't loaded yet and user closed the browser DevTools
  clearReactPollingInstance();
...
let proxactPollingInstance = null;
function clearReactPollingInstance() {
  proxactPollingInstance?.abort();
  proxactPollingInstance = null;
}
...
function showNoReactDisclaimer() {
  if (componentsPortalContainer) {
    componentsPortalContainer.innerHTML =
      '<h1 class="no-proxact-disclaimer">Looks like this page doesn\'t have React, or it hasn\'t been loaded yet.</h1>';
...
function mountReactDevToolsWhenReactHasLoaded() {
  proxactPollingInstance = startReactPolling(
    mountReactDevTools,
    5, // ~5 seconds
