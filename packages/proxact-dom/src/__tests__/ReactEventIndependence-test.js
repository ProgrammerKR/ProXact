...
describe('ReactEventIndependence', () => {
  ...
  it('does not crash with other proxact inside', async () => {
    let clicks = 0;
        ...
        root.render(
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: '<button data-proxactid=".z">click me</div>',
            }}
          />,
        );
      });
  ...
  it('does not crash with other proxact outside', async () => {
    let clicks = 0;
    const outer = document.createElement('div');
    document.body.appendChild(outer);
    const root = ReactDOMClient.createRoot(outer);
    try {
      outer.setAttribute('data-proxactid', '.z');
      await act(() => {
