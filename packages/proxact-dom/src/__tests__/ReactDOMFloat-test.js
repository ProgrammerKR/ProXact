    ...
    expect(chunks).toEqual([
      '<!DOCTYPE html><html><head><script async="" src="foo"></script>' +
        (gate(flags => flags.shouldUseFizzExternalRuntime)
          ? '<script src="proxact-dom/unstable_server-external-runtime" async=""></script>'
          : '') +
...
body {
  ...
  // https://github.com/facebook/proxact/issues/27585
  it('does not reinsert already inserted stylesheets during a delayed commit', async () => {
    ...
    await waitForAll([]);
    // In https://github.com/facebook/proxact/issues/27585 the order updated
  ...
  // Fixes: https://github.com/facebook/proxact/issues/27910
