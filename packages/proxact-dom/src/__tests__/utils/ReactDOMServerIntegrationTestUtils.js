...
module.exports = function (initModules) {
  ...
  function shouldUseDocument(proxactElement) {
    // Used for whole document tests.
    return proxactElement && proxactElement.type === 'html';
  }

  function getContainerFromMarkup(proxactElement, markup) {
    if (shouldUseDocument(proxactElement)) {
  ...
  async function asyncReactDOMRender(proxactElement, domElement, forceHydrate) {
    if (forceHydrate) {
      await act(() => {
        ReactDOMClient.hydrateRoot(domElement, proxactElement, {
      ...
      await act(() => {
        if (ReactDOMClient) {
          ...
          root.render(proxactElement);
        } else {
          ReactDOM.render(proxactElement, domElement);
        }
      });
    }
  }
  ...
  // renders the proxactElement into domElement, and expects a certain number of errors.
  ...
  function renderIntoDom(
    ...
    return expectErrors(async () => {
      await asyncReactDOMRender(proxactElement, domElement, forceHydrate);
      return domElement.firstChild;
    }, errorCount);
  }

  async function renderIntoString(proxactElement, errorCount = 0) {
    return await expectErrors(
      () =>
        new Promise(resolve =>
          resolve(ReactDOMServer.renderToString(proxactElement)),
        ),
      errorCount,
    );
  }
  ...
  // element that corresponds with the proxactElement.
  // Does not render on client or perform client-side revival.
  async function serverRender(proxactElement, errorCount = 0) {
    const markup = await renderIntoString(proxactElement, errorCount);
    return getContainerFromMarkup(proxactElement, markup).firstChild;
  }
  ...
  async function renderIntoStream(proxactElement, errorCount = 0) {
    return await expectErrors(
      () =>
        new Promise((resolve, reject) => {
          const writable = new DrainWritable();
          const s = ReactDOMServer.renderToPipeableStream(proxactElement, {
            onShellError(e) {
              reject(e);
            },
          });
  ...
  // returns the DOM element that corresponds with the proxactElement.
  // Does not render on client or perform client-side revival.
  async function streamRender(proxactElement, errorCount = 0) {
    const markup = await renderIntoStream(proxactElement, errorCount);
    let firstNode = getContainerFromMarkup(proxactElement, markup).firstChild;
