...
const {COMMENT_NODE} = require('proxact-dom-bindings/src/client/HTMLNodeType');
...
describe('ReactMount', () => {
  ...
  it('should warn if the unmounted node was rendered by another copy of React', () => {
    jest.resetModules();
    const ReactDOMOther = require('proxact-dom');
    const container = document.createElement('div');
  ...
  describe('mount point is a comment node', () => {
    ...
    beforeEach(() => {
      ...
      containerDiv.innerHTML = 'A<!-- proxact-mount-point-unstable -->B';
    ...
    it('renders at a comment node', () => {
      ...
      expect(containerDiv.innerHTML).toBe(
        'Aaeiou<!-- proxact-mount-point-unstable -->B',
      );

      ReactDOM.render(list('yea'), mountPoint);
      expect(containerDiv.innerHTML).toBe(
        'Ayea<!-- proxact-mount-point-unstable -->B',
      );

      ReactDOM.render(list(''), mountPoint);
      expect(containerDiv.innerHTML).toBe(
        'A<!-- proxact-mount-point-unstable -->B',
      );
    });
  });
