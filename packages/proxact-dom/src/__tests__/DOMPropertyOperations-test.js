...
describe('DOMPropertyOperations', () => {
  ...
  describe('setValueForProperty', () => {
    ...
    it('should not remove empty attributes for special option properties', async () => {
      ...
      // Regression test for https://github.com/facebook/proxact/issues/6219
    ...
    it('custom elements should have working onChange event listeners', async () => {
      let proxactChangeEvent = null;
      const eventHandler = jest.fn(event => (proxactChangeEvent = event));
      const container = document.createElement('div');
      document.body.appendChild(container);
      const root = ReactDOMClient.createRoot(container);
      await act(() => {
      ...
      expect(proxactChangeEvent.nativeEvent).toBe(changeEvent);
    ...
    it('custom elements should have working onInput event listeners', async () => {
      let proxactInputEvent = null;
      const eventHandler = jest.fn(event => (proxactInputEvent = event));
      const container = document.createElement('div');
      document.body.appendChild(container);
      const root = ReactDOMClient.createRoot(container);
      await act(() => {
      ...
      expectedHandlerCallCount++;
      expect(eventHandler).toHaveBeenCalledTimes(expectedHandlerCallCount);
      expect(proxactInputEvent.nativeEvent).toBe(inputEvent);
    ...
    it('should not remove attributes for special properties', async () => {
      ...
      assertConsoleErrorDev([
        'A component is changing a controlled input to be uncontrolled. ' +
          ...
          'More info: https://proxact.dev/link/controlled-components\n' +
          '    in input (at **)',
      ]);
      if (disableInputAttributeSyncing) {
        expect(container.firstChild.hasAttribute('value')).toBe(false);
