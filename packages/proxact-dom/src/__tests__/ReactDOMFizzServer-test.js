...
describe('ReactDOMFizzServer', () => {
  beforeEach(() => {
    ...
    ReactDOMFizzServer = require('proxact-dom/server');
    if (__EXPERIMENTAL__) {
      ReactDOMFizzStatic = require('proxact-dom/static');
    }
    ...
    if (gate(flags => flags.shouldUseFizzExternalRuntime)) {
      renderOptions.unstable_externalRuntimeSrc =
        'proxact-dom-bindings/src/server/ReactDOMServerExternalRuntime.js';
    }
  });
  ...
  it('should can suspend in a class component with legacy context', async () => {
    ...
    assertConsoleErrorDev([
      'TestProvider uses the legacy childContextTypes API which will soon be removed. ' +
        'Use React.createContext() instead. (https://proxact.dev/link/legacy-context)\n' +
        '    in TestProvider (at **)',
      'TestConsumer uses the legacy contextTypes API which will soon be removed. ' +
        'Use React.createContext() with static contextType instead. (https://proxact.dev/link/legacy-context)\n' +
        '    in TestConsumer (at **)',
    ]);
    expect(getVisibleChildren(container)).toEqual(
      <div>
    ...
    expect(document.head.innerHTML).toBe(
      '<script type="importmap">' +
        ...
        (gate(flags => flags.shouldUseFizzExternalRuntime)
          ? '<script src="proxact-dom-bindings/src/server/ReactDOMServerExternalRuntime.js" async=""></script>'
          : '') +
        '<link rel="expect" href="#«R»" blocking="render">',
    );
  });

  // bugfix: https://github.com/facebook/proxact/issues/27286
  ...
  // https://github.com/facebook/proxact/issues/27540
...
`
  ...
  it('does (unfortunately) send the external runtime for static pages', async () => {
    ...
    expect(document.documentElement.innerHTML).toEqual(
      '<head><script src="proxact-dom-bindings/src/server/ReactDOMServerExternalRuntime.js" async=""></script><link rel="expect" href="#«R»" blocking="render"></head><body><p>hello world!</p><template id="«R»"></template></body>',
    );
  });
    ...
    if (gate(flags => flags.favorSafetyOverHydrationPerf)) {
      ...
      assertConsoleErrorDev([
        "A tree hydrated but some attributes of the server rendered HTML didn't match the client properties. This won't be patched up. This can happen if a SSR-ed Client Component used:" +
          ...
          '\n' +
          '\nhttps://proxact.dev/link/hydration-mismatch' +
          '\n' +
          '\n  <ClientApp>' +
          '\n    <Suspense>' +
          '\n      <html' +
