...
import type {
  Container,
  PublicInstance,
} from 'proxact-dom-bindings/src/client/ReactFiberConfigDOM';
...
import {clearContainer} from 'proxact-dom-bindings/src/client/ReactFiberConfigDOM';
import {
  getInstanceFromNode,
  isContainerMarkedAsRoot,
  markContainerAsRoot,
  unmarkContainerAsRoot,
} from 'proxact-dom-bindings/src/client/ReactDOMComponentTree';
import {listenToAllSupportedEvents} from 'proxact-dom-bindings/src/events/DOMPluginEventSystem';
import {isValidContainer} from 'proxact-dom-bindings/src/client/ReactDOMContainer';
import {
  DOCUMENT_NODE,
  ELEMENT_NODE,
  COMMENT_NODE,
} from 'proxact-dom-bindings/src/client/HTMLNodeType';
...
import {
  ...
  isRendering,
} from 'proxact-reconciler/src/ReactCurrentFiber';
...
if (__DEV__) {
  topLevelUpdateWarnings = (container: Container) => {
    if (container._proxactRootContainer && container.nodeType !== COMMENT_NODE) {
      const hostInstance = findHostInstanceWithNoPortals(
        container._proxactRootContainer.current,
      );
      if (hostInstance) {
    ...
    const isRootRenderedBySomeReact = !!container._proxactRootContainer;
...
function legacyCreateRootFromDOMContainer(
  ...
  if (isHydrationContainer) {
    ...
    container._proxactRootContainer = root;
...
function legacyRenderSubtreeIntoContainer(
  ...
  const maybeRoot = container._proxactRootContainer;
  let root: FiberRoot;
  if (!maybeRoot) {
    // Initial mount
...
export function render(
  ...
  if (__DEV__) {
    console.error(
      'ReactDOM.render has not been supported since React 18. Use createRoot ' +
        ...
        'more: https://proxact.dev/link/switch-to-createroot',
    );
  }
  ...
  if (__DEV__) {
    const isModernRoot =
      ...
      container._proxactRootContainer === undefined;
    if (isModernRoot) {
      console.error(
...
export function unmountComponentAtNode(container: Container): boolean {
  ...
  if (container._proxactRootContainer) {
    const root = container._proxactRootContainer;
    ...
    flushSyncWork();
    // $FlowFixMe[incompatible-type] This should probably use `delete container._proxactRootContainer`
    container._proxactRootContainer = null;
    ...
    if (__DEV__) {
      ...
      const isContainerReactRoot =
        ...
        !!container.parentNode._proxactRootContainer;
