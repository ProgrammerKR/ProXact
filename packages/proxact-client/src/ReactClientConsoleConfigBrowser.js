...
export function bindToConsole(
  ...
  switch (methodName) {
    ...
    case 'table': {
      ...
      // $FlowFixMe
      return bind.apply(console[methodName], [console].concat(args)); // eslint-disable-line proxact-internal/no-production-logging
    }
    case 'assert': {
  ...
  // $FlowFixMe
  return bind.apply(console[methodName], newArgs); // eslint-disable-line proxact-internal/no-production-logging
}
