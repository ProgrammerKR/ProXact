...
describe('ReactFlight', () => {
  beforeEach(() => {
    ...
    ReactServer = require('proxact');
    ReactNoopFlightServer = require('proxact-noop-renderer/flight-server');
    // This stores the state so we need to preserve it
    const flightModules = require('proxact-noop-renderer/flight-modules');
    jest.resetModules();
    __unmockReact();
    jest.mock('proxact-noop-renderer/flight-modules', () => flightModules);
    React = require('proxact');
    startTransition = React.startTransition;
    use = React.use;
    ReactNoop = require('proxact-noop-renderer');
    ReactNoopFlightClient = require('proxact-noop-renderer/flight-client');
  ...
  function clientReference(value) {
    return Object.defineProperties(
      ...
      {
        $$typeof: {value: Symbol.for('proxact.client.reference')},
        value: {value: value},
      },
    );
  }
  ...
  it('should emit descriptions of errors in dev', async () => {
    ...
    function RenderInlined() {
      const inlinedElement = {
        $$typeof: Symbol.for('proxact.element'),
        type: () => {},
        key: null,
        ref: null,
        props: {},
      ...
      <>
        ...
        <ClientErrorBoundary
          expectedMessage={
            ...
            '- Multiple copies of the "proxact" package is used.\n' +
            '- A library pre-bundled an old copy of "proxact" or "proxact/jsx-runtime".\n' +
  ...
  it('should handle exotic stack frames', async () => {
    function ServerComponent() {
      ...
      error.stack = [
        ...
        // original
        // '    at ServerComponentError (file://~/proxact/packages/proxact-client/src/__tests__/ReactFlight-test.js:1166:19)',
  ...
  it('should include server components in warning stacks', async () => {
    ...
    assertConsoleErrorDev([
      'Each child in a list should have a unique "key" prop.\n' +
        '\n' +
        'Check the render method of `Component`. See https://proxact.dev/link/warning-keys for more information.\n' +
        '    in span (at **)\n' +
        '    in Component (at **)\n' +
        '    in App (at **)',
    ]);
  ...
  it('should warn in DEV a child is missing keys on server component', () => {
    ...
    jest.resetModules();
    jest.mock('proxact', () => React);
    ReactNoopFlightClient.read(transport);
    assertConsoleErrorDev([
      'Each child in a list should have a unique "key" prop. ' +
        'See https://proxact.dev/link/warning-keys for more information.\n' +
        '    in NoKey (at **)',
      'Each child in a list should have a unique "key" prop. ' +
        'See https://proxact.dev/link/warning-keys for more information.\n' +
        '    in NoKey (at **)',
  ...
  it('can track owner for a flight response created in another render', async () => {
    jest.resetModules();
    jest.mock('proxact', () => ReactServer);
    // For this to work the Flight Client needs to be the proxact-server version.
    const ReactNoopFlightClienOnTheServer = require('proxact-noop-renderer/flight-client');
    jest.resetModules();
    jest.mock('proxact', () => React);
