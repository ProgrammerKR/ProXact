...
## Installation
Install the `proxact-devtools` package. Because this is a development tool, a global install is often the most convenient:
```sh
# Yarn
yarn global add proxact-devtools

# NPM
npm install -g proxact-devtools
```

If you prefer to avoid global installations, you can add `proxact-devtools` as a project dependency. With Yarn, you can do this by running:
```sh
yarn add --dev proxact-devtools
```

With NPM you can just use [NPX](https://www.npmjs.com/package/npx):
```sh
npx proxact-devtools
```

## Usage with React Native
Run `proxact-devtools` from the terminal to launch the standalone DevTools app:
```sh
proxact-devtools
```
...
You can open the [in-app developer menu](https://proxactnative.dev/docs/debugging#accessing-the-in-app-developer-menu) and choose "Show Inspector". It will bring up an overlay that lets you tap on any UI element and see information about it:

![React Native Inspector](https://i.imgur.com/ReFhREb.gif)

However, when `proxact-devtools` is running, Inspector will enter a special collapsed mode, and instead use the DevTools as primary UI. In this mode, clicking on something in the simulator will bring up the relevant components in the DevTools:
...
First, follow the [instructions for debugging in Chrome](https://proxactnative.dev/docs/debugging#chrome-developer-tools) to open the Chrome console.
...
>If you install `proxact-devtools` as a project dependency, you may also replace the `<script>` suggested above with a JavaScript import (`import 'proxact-devtools'`). It is important that this import comes before any other imports in your app (especially before `proxact-dom`). Make sure to remove the import before deploying to production, as it carries a large DevTools client with it. If you use Webpack and have control over its configuration, you could alternatively add `'proxact-devtools'` as the first item in the `entry` array of the development-only configuration, and then you wouldn’t need to deal either with `<script>` tags or `import` statements.

## Advanced

By default DevTools listen to port `8097` on `localhost`. The port can be modified by setting the `REACT_DEVTOOLS_PORT` environment variable. If you need to further customize host, port, or other settings, see the `proxact-devtools-core` package instead.
...
**If your app is inside an iframe, a Chrome extension, React Native, or in another unusual environment**, try [the standalone version instead](https://github.com/facebook/proxact/tree/main/packages/proxact-devtools). Chrome apps are currently not inspectable.

**If your Components tab is empty, refer to "The React tab shows no components" section below**.

**If you still have issues** please [report them](https://github.com/facebook/proxact/issues/new?labels=Component:%20Developer%20Tools). Don't forget to specify your OS, browser version, extension version, and the exact instructions to reproduce the issue with a screenshot.
...
* Then, run `yarn start:backend` and `yarn start:standalone` in `packages/proxact-devtools-core`
* Run `yarn start` in `packages/proxact-devtools`
* Refresh the app after it has recompiled a change
* For React Native, copy `proxact-devtools-core` to its `node_modules` to test your changes.
