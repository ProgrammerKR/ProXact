...
If you're looking for the standalone React DevTools UI, **we suggest using [`proxact-devtools`](https://github.com/facebook/proxact/tree/main/packages/proxact-devtools) instead of using this package directly**.

---

> **Note** that this package (and the DevTools UI) relies on several _experimental_ APIs that are **only available in the [experimental release channel](https://proxactjs.org/docs/release-channels.html#experimental-channel)**. This means that you will need to install `proxact@experimental` and `proxact-dom@experimental`.
...
### Example
```js
import { activate, initialize } from 'proxact-devtools-inline/backend';
...
### Example
```js
import { initialize } from 'proxact-devtools-inline/frontend';
...
const hookNamesModuleLoaderFunction = () => import('proxact-devtools-inline/hookNames');
...
import { initialize as initializeFrontend } from 'proxact-devtools-inline/frontend';
...
} from 'proxact-devtools-inline/frontend';
...
const {
  ...
  createBridge,
  initialize,
} = require('proxact-devtools-inline/backend');
...
To test package changes, refer to the [`proxact-devtools-shell` README](https://github.com/facebook/proxact/blob/main/packages/proxact-devtools-shell/README.md).
