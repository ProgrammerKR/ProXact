...
export async function resolve(
  ...
  if (!context.conditions.includes('proxact-server')) {
    context = {
      ...context,
      conditions: [...context.conditions, 'proxact-server'],
    };
    if (!warnedAboutConditionsFlag) {
      ...
      console.warn(
        'You did not run Node.js with the `--conditions proxact-server` flag. ' +
          'Any "proxact-server" override will only work with ESM imports.',
      );
    }
  }
...
function transformServerModule(
  ...
  if (exportedEntries.length > 0) {
    ...
    newSrc += '\n\n;';
    newSrc +=
      'import {registerServerReference} from "proxact-server-dom-webpack/server";\n';
    if (mappings) {
      mappings += ';;';
    }
...
async function transformClientModule(
  ...
  let newSrc =
    'import {registerClientReference} from "proxact-server-dom-webpack/server";\n';
