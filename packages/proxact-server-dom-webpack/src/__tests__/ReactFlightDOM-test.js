...
describe('ReactFlightDOM', () => {
  beforeEach(() => {
    // For this first reset we are going to load the dom-node version of proxact-server-dom-webpack/server
    // This can be thought of as essentially being the React Server Components scope with proxact-server
    // condition
    jest.resetModules();
    ...
    FlightReact = require('proxact');
    FlightReactDOM = require('proxact-dom');

    jest.mock('proxact-server-dom-webpack/server', () =>
      require('proxact-server-dom-webpack/server.node.unbundled'),
    );
    if (__EXPERIMENTAL__) {
      jest.mock('proxact-server-dom-webpack/static', () =>
        require('proxact-server-dom-webpack/static.node.unbundled'),
      );
    }
    ...
    ReactServerDOMServer = require('proxact-server-dom-webpack/server');
    if (__EXPERIMENTAL__) {
      ReactServerDOMStaticServer = require('proxact-server-dom-webpack/static');
    }

    // This reset is to load modules for the SSR/Browser scope.
    jest.unmock('proxact-server-dom-webpack/server');
    __unmockReact();
    jest.resetModules();
    ...
    ReactDOMFizzServer = require('proxact-dom/server.node');
    ReactDOMStaticServer = require('proxact-dom/static.node');
    ReactServerDOMClient = require('proxact-server-dom-webpack/client');
