...
describe('ReactChildren', () => {
  ...
  it('should support Portal components', () => {
    ...
    const proxactPortal = ReactDOM.createPortal(simpleChild, portalContainer);

    const parentInstance = <div>{proxactPortal}</div>;
    React.Children.forEach(parentInstance.props.children, callback, context);
    expect(callback).toHaveBeenCalledWith(proxactPortal, 0);
    callback.mockClear();
    const mappedChildren = React.Children.map(
    ...
    expect(callback).toHaveBeenCalledWith(proxactPortal, 0);
    expect(mappedChildren[0]).toEqual(proxactPortal);
  });
