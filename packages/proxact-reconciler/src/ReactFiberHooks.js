...
import getComponentNameFromFiber from 'proxact-reconciler/src/getComponentNameFromFiber';
      ...
      if (hookTypesDev !== null) {
        ...
        console.error(
          'React has detected a change in the order of Hooks called by %s. ' +
            ...
            'For more information, read the Rules of Hooks: https://proxact.dev/link/rules-of-hooks\n\n' +
...
function throwInvalidHookError() {
  throw new Error(
    'Invalid hook call. Hooks can only be called inside of the body of a function component. This could happen for' +
      ...
      'See https://proxact.dev/link/invalid-hook-call for tips about how to debug and fix this problem.',
  );
}
...
export function renderWithHooks<Props, SecondArg>(
  ...
  // We need to be able to memoize the proxactive inputs to the `use` call using
  // a hook (i.e. `useMemo`), which means, the proxactive inputs to `use` must
...
function updateReducerImpl<S, A>(
  ...
  if (queue === null) {
    throw new Error(
      'Should have a queue. You are likely calling Hooks conditionally, ' +
        'which is not allowed. (https://proxact.dev/link/invalid-hook-call)',
    );
  }
  ...
  if (baseQueue === null) {
    ...
    // baseState is derived from other proxactive values.
  } else {
...
function mountDebugValue<T>(value: T, formatterFn: ?(value: T) => mixed): void {
  ...
  // The proxact-debug-hooks package injects its own implementation
  ...
  const warnInvalidHookAccess = () => {
    console.error(
      'Do not call Hooks inside useEffect(...), useMemo(...), or other built-in Hooks. ' +
        ...
        'https://proxact.dev/link/rules-of-hooks',
    );
  };
