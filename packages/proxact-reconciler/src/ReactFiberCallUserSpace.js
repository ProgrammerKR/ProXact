...
const callComponent = {
  'proxact-stack-bottom-frame': function <Props, Arg, R>(
...
export const callComponentInDEV: <Props, Arg, R>(
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (callComponent['proxact-stack-bottom-frame'].bind(callComponent): any)
  : (null: any);
...
const callRender = {
  'proxact-stack-bottom-frame': function <R>(instance: ClassInstance<R>): R {
    const wasRendering = isRendering;
    setIsRendering(true);
    try {
...
export const callRenderInDEV: <R>(instance: ClassInstance<R>) => R => R =
  __DEV__
    ? // We use this technique to trick minifiers to preserve the function name.
      (callRender['proxact-stack-bottom-frame'].bind(callRender): any)
    : (null: any);

const callComponentDidMount = {
  'proxact-stack-bottom-frame': function (
    finishedWork: Fiber,
...
export const callComponentDidMountInDEV: (
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (callComponentDidMount['proxact-stack-bottom-frame'].bind(
...
export const callComponentDidUpdateInDEV: (
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (callComponentDidUpdate['proxact-stack-bottom-frame'].bind(
...
export const callComponentDidCatchInDEV: (
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (callComponentDidCatch['proxact-stack-bottom-frame'].bind(
...
export const callComponentWillUnmountInDEV: (
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (callComponentWillUnmount['proxact-stack-bottom-frame'].bind(
...
export const callCreateInDEV: (effect: Effect) => (() => void) | void = __DEV__
  ? // We use this technique to trick minifiers to preserve the function name.
    (callCreate['proxact-stack-bottom-frame'].bind(callCreate): any)
  : (null: any);
...
export const callDestroyInDEV: (
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (callDestroy['proxact-stack-bottom-frame'].bind(callDestroy): any)
  : (null: any);

const callLazyInit = {
  'proxact-stack-bottom-frame': function (lazy: LazyComponent<any, any>): any {
...
export const callLazyInitInDEV: (lazy: LazyComponent<any, any>) => any = __DEV__
  ? // We use this technique to trick minifiers to preserve the function name.
    (callLazyInit['proxact-stack-bottom-frame'].bind(callLazyInit): any)
  : (null: any);
