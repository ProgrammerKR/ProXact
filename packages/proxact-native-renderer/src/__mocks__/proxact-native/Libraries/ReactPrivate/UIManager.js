...
const RCTUIManager = {
  ...
  createView: jest.fn(function createView(proxactTag, viewName, rootTag, props) {
    if (views.has(proxactTag)) {
      throw new Error(`Created two native views with tag ${proxactTag}`);
    }

    views.set(proxactTag, {
      children: [],
      parent: null,
      props: props,
      viewName: viewName,
  ...
  setChildren: jest.fn(function setChildren(parentTag, proxactTags) {
