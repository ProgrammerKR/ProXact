...
const RCTFabricUIManager = {
  ...
  createNode: jest.fn(
    function createNode(proxactTag, viewName, rootTag, props, eventTarget) {
      if (allocatedTags.has(proxactTag)) {
        throw new Error(`Created two native views with tag ${proxactTag}`);
      }

      allocatedTags.add(proxactTag);
      return {
        proxactTag: proxactTag,
        viewName: viewName,
        props: props,
        children: [],
      };
