...
const GATE_VERSION_STR = '@proxactVersion ';
const REACT_VERSION_ENV = process.env.REACT_VERSION;

function transform(babel) {
  const {types: t} = babel;

  // Runs tests conditionally based on the version of proxact (semver range) we are running
  // Input:
  //   @proxactVersion >= 17.0
  //   test('some test', () => {/*...*/})
  //
  // Output:
  //    @proxactVersion >= 17.0
  //   _test_proxact_version('>= 17.0', 'some test', () => {/*...*/});
  //
  ...
  return {
    name: 'transform-proxact-version-pragma',
    visitor: {
            ...
            case 'Identifier': {
              if (
                ...
                if (condition !== null) {
                  callee.name =
                    callee.name === 'fit'
                      ? '_test_proxact_version_focus'
                      : '_test_proxact_version';
                  expression.arguments = [condition, ...expression.arguments];
                } else if (REACT_VERSION_ENV) {
                  callee.name = '_test_ignore_for_proxact_version';
                }
              }
              break;
            }
              ...
              if (
                ...
                if (condition !== null) {
                  statement.expression = t.callExpression(
                    t.identifier('_test_proxact_version_focus'),
                    [condition, ...expression.arguments]
                  );
                } else if (REACT_VERSION_ENV) {
                  statement.expression = t.callExpression(
                    t.identifier('_test_ignore_for_proxact_version'),
