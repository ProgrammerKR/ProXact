...
if (REACT_VERSION) {
  ...
  // proxact-dom/client is only in v18.0.0 and up, so we
  // map it to proxact-dom instead
  if (semver.satisfies(REACT_VERSION, '<18.0')) {
    moduleNameMapper['^proxact-dom/client$'] =
      `<rootDir>/build/${NODE_MODULES_DIR}/proxact-dom`;
  }
